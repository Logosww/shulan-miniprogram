<template>
  <ConfigProvider>
    <Container navbar-title="报名详情" :bg-style="{ transition: 'background-color .25s', transitionDelay: '.2s', backgroundColor: isLoading ? '#fff' : '#50FE82' }" disable-safe-bottom>
      <div class="relative h-full rounded-t-[16px] overflow-hidden">
        ...
        >
          <div>
            <div class="px-[16px] py-[20px] bg-white mb-[16px]">
              <div class="flex">
                <image class="w-[94px] h-[128px] rounded-[12px] mr-[12px] flex-shrink-0" mode="aspectFill" :src="data?.activity.coverUrl" :fade-in="true" @tap="coverPreviewerVisible = true" />
                <div class="flex flex-col justify-between">
                  ...
                  <div class="text-[12px] text-[#666] leading-[17px]">
                    ...
                    <div class="flex items-center" v-if="data?.activityWorkVolunteerIdentity === VolunteerType.normal">
                      <image class="w-[12px] h-[12px] mr-[4px] flex-shrink-0" src="@/assets/icon/home/time.svg" mode="aspectFit" :svg="true" />
                      工作时间：
                    </div>
              ...
              <div class="text-[#0D0F02] text-[16px] leading-[22px] flex justify-between">
                ...
                <div class="flex items-center">
                  <div v-if="data?.activity.isMoney">￥{{ data?.work.money }}</div>
                  <image class="w-[14px] h-[14px] ml-[16px] mr-[4px]" src="@/assets/icon/integral.svg" mode="aspectFit" :svg="true" />
                  +{{ data?.work.integral }}
                </div>
              </div>
              <div class="my-[12px] h-[1px] bg-[#E6E6E6]"></div>
              ...
              <div class="flex justify-between items-center" @tap="anouncementModalVisible = true">
                <div class="flex items-center">
                  ...
                  <div class="text-[#0D0F02] text-[14px] leading-[16px] max-w-[252px] truncate">{{ data?.activity.announcement || '暂无公告' }}</div>
                </div>
            ...
            <div class="bg-white py-[20px] px-[16px] mb-[16px]">
              <image class="w-[92px] h-[15px] mb-[12px]" src="@/assets/icon/sign-up-detail/check-in.svg" mode="aspectFit" :svg="true" />
              ...
              <div class="mt-[4px]" v-if="data?.checkin && !data?.checkin.isChecked">
                <div class="mb-[12px] text-[#B3B3B3] text-[12px] leading-[17px]">请现场给活动工作人员进行扫码签到</div>
        ...
        <nut-image-preview :show="coverPreviewerVisible" :images="[{ src: data?.activity.coverUrl ?? '' }]" :fade-in="true" @close="coverPreviewerVisible = false" />
      </div>
    </Container>
  </ConfigProvider>
</template>
