  ...
  >
    ...
    <template #navbar-bottom>
      <div class="w-full h-[42px] pt-[4px] pb-[8px] px-[16px] left-0 flex items-center box-border">
        <div class="flex items-center" @tap="handleToSelectCity">
          <image class="w-[16px] h-[16px] mr-[4px]" src="@/assets/icon/home/position.svg" mode="aspectFit" :svg="true" />
        ...
        <div class="py-[7px] pl-[10px] flex ml-[16px] mr-[15px] bg-white border-[#8ECCA0] border-[1px] border-solid rounded-[20px] flex-[1]" @tap="Taro.navigateTo({ url: '/packageB/pages/search/search' })">
          <image class="w-[16px] h-[16px] mr-[12px]" src="@/assets/icon/home/search.svg" mode="aspectFit" :svg="true" />
        </div>
        <NotificationButton />
      </div>
    </template>
    ...
    >
      <nut-swiper class="h-[150px] rounded-[8px] bg-white" :auto-play="3000" pagination-visible @change="index => currBannerIndex = index">
        <nut-swiper-item v-for="(banner, index) in bannerList" :key="index" @tap="handleBannerTap(banner)">
          <image class="w-full h-full" mode="aspectFill" :src="banner.coverUrl" :fade-in="true" />
        </nut-swiper-item>
        <template #page>
      ...
      <div class="pt-[13px] pb-[9px] px-[19px] my-[16px] bg-white rounded-[12px] flex justify-between">
        <div @tap="Taro.navigateTo({ url: `/packageB/pages/activity-list/activity-list?type=${ActivityType.concert}` })">
          <image class="w-[40px] h-[40px] mb-[4px]" src="@/assets/icon/home/concert.svg" mode="aspectFit" :svg="true" />
        ...
        <div @tap="Taro.navigateTo({ url: `/packageB/pages/activity-list/activity-list?type=${ActivityType.drama}` })">
          <image class="w-[40px] h-[40px] mb-[4px]" src="@/assets/icon/home/drama.svg" mode="aspectFit" :svg="true" />
        ...
        <div @tap="Taro.navigateTo({ url: `/packageB/pages/activity-list/activity-list?type=${ActivityType.talkshow}` })">
          <image class="w-[40px] h-[40px] mb-[4px]" src="@/assets/icon/home/talkshow.svg" mode="aspectFit" :svg="true" />
        ...
        <div @tap="Taro.navigateTo({ url: `/packageB/pages/activity-list/activity-list?type=${ActivityType.musicFesitival}` })">
          <image class="w-[40px] h-[40px] mb-[4px]" src="@/assets/icon/home/music_festival.svg" mode="aspectFit" :svg="true" />
        ...
        <div @tap="Taro.navigateTo({ url: `/packageB/pages/activity-list/activity-list?type=${ActivityType.sports}` })">
          <image class="w-[40px] h-[40px] mb-[4px]" src="@/assets/icon/home/sports.svg" mode="aspectFit" :svg="true" />
          <div class="text-center text-black text-[14px] font-[500] leading-[16px]">体育</div>
        </div>
      </div>
      <div class="text-black text-[20px] leading-[24px] mb-[12px] font-bold before:absolute">
...
const fetchActivityList = useThrottleFn(async (page?: number, city?: string) => {
...
const handleToSelectCity = async () => {
  Taro.showLoading({ title: '加载中' });
  const cityList = await downloadAndRead<IElevatorDataItem[]>({ key: 'cityList.json', jsonParse: true })
    .then(({ data: cityList }) => cityList)
    .finally(() => setTimeout(() => Taro.hideLoading()));
      
  const { authSetting } = await Taro.getSetting();
  if(!authSetting['scope.userFuzzyLocation']) {
    Taro.preload({ cityList, currentCity: void 0 });
    return Taro.navigateTo({ url: '/packageB/pages/select-city/select-city' });
  }
